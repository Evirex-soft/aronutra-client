...
export default async function AllOrdersPage({
    ...
    const getStatusStyles = (status: string) => {
        switch (status.toLowerCase()) {
            ...
            default: return 'text-[#d4af37] bg-[#d4af37]/10';
        }
    };
                    ...
                    {orders.length > 0 ? (
                        orders.map((order: any) => (
                            <Link
                                ...
                                className="group block bg-white/[0.02] border border-white/10 hover:border-[#d4af37]/40 rounded-3xl p-5 md:p-6 transition-all duration-500 hover:bg-white/[0.04]"
                            >
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                                    {/* Order Main Info */}
                                    <div className="flex items-center gap-5">
                                        <div className="relative h-16 w-16 hidden sm:block">
                                            <div className="absolute inset-0 bg-[#d4af37]/20 rounded-2xl rotate-6 group-hover:rotate-12 transition-transform" />
                                            <div className="absolute inset-0 bg-white/10 backdrop-blur-sm rounded-2xl flex items-center justify-center border border-white/10">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-3 mb-1">
                                                ...
                                                <span className={`text-[8px] uppercase tracking-widest px-2 py-0.5 rounded-full font-bold ${getStatusStyles(order.status)}`}>
                                                    {order.status}
                                                </span>
                                            </div>
                                    ...
                                    {/* Right Section: Thumbnails & Price */}
                                    <div className="flex items-center justify-between md:justify-end gap-8">
                                        {/* Item Overlap Thumbnails (Luxury touch) */}
                                        <div className="hidden lg:flex -space-x-3">
                                            ...
                                            {order.items.length > 3 && (
                                                <div className="w-8 h-8 rounded-full border-2 border-[#052c22] bg-white/10 flex items-center justify-center text-[8px]">
                                                    +{order.items.length - 3}
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        <div className="text-left md:text-right">
                                            ...
                                            <p className="text-[9px] uppercase tracking-[0.2em] text-white/40">{order.items.length} {order.items.length === 1 ? 'Item' : 'Items'}</p>
                                        </div>
